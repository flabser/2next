...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="doctype" select="'settings'" />
				...
				<tr class="gray-border-table">
					...
					<td style="padding-left:5px">
						...
						<img style="margin-left:10px; cursor:pointer" src="img/force_small.gif">
							<xsl:attribute name="onclick">javascript:updateDBver('<xsl:value-of select="query/database/version" />','<xsl:value-of
								select="query/application" />')</xsl:attribute>
						</img>
					</td>
				</tr>
				<tr class="gray-border-table">
						...
						<td style="padding-left:5px">
							<a style="text-decoration:none">
								<xsl:if test="lastnum != 0">
									<xsl:attribute name="href">javascript:changeRegNumber('<xsl:value-of select="keys" />',<xsl:value-of
										select="lastnum - 1" />,'<xsl:value-of select="$dbid" />')</xsl:attribute>
								</xsl:if>
								-
							</a>
