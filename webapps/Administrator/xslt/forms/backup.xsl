...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<head>
			<title>Administrator - Backup</title>
		...
		<body>
				...
				<tr>
					<td width="50%">
						...
						<a style="margin-left:10px">
							<xsl:attribute name="href">javascript:restore_backup('<xsl:value-of select="document/id"/>','<xsl:value-of select="document/app"/>');</xsl:attribute>
							<img src="img/refresh.gif"/>
							<font class="button">Restore backup</font>
						</a>
						<a style="margin-left:10px">
			...
			<br/>
			<font style="font-family:verdana; font-size:20px;"><xsl:value-of select="concat('Backup -', document/id)"/></font>
			<br/>
			<hr/>
