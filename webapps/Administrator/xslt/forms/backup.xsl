...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="_content">
			...
			<tr>
				<td width="50%">
					...
					<a style="margin-left:10px">
						<xsl:attribute name="href">javascript:restore_backup('<xsl:value-of select="document/id" />','<xsl:value-of
							select="document/app" />');</xsl:attribute>
		...
		<font style="font-family:verdana; font-size:20px;">
			<xsl:value-of select="concat('Backup -', document/id)" />
		</font>
		<br />
		<hr />
