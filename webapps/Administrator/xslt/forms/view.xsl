...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<xsl:call-template name="layout">
			<xsl:with-param name="w_title" select="concat('Administrator - View', rule/@id)" />
		</xsl:call-template>
	</xsl:template>
		...
		<table width="100%">
			<tr>
				<td width="50%">
					...
					<a style="margin-left:10px">
						<xsl:attribute name="href">javascript:saveRule('<xsl:value-of select="rule/@id" />', '<xsl:value-of
							select="rule/@app" />','<xsl:value-of select="@dbid" />')</xsl:attribute>
					...
					<td>
						<input type="radio" name="doctype" value="DOCUMENT">
							<xsl:attribute name="onclick">
									javascript:changeTypeQuery("<xsl:value-of select="rule/@id" />","<xsl:value-of
								select="rule/@app" />","<xsl:value-of select="$dbid" />")
								</xsl:attribute>
										...
										<xsl:for-each select="rule/glossaries/rununderusersource/query/entry">
											<option value="{@viewtext}">
												<xsl:if test="$rununderusersource = @viewtext">
													<xsl:attribute name="selected" select="'seleted'" />
												</xsl:if>
