	...
	<xsl:template match="/">
		...
		<table class="viewtable">
			<tr class="th">
				...
				<td width="15%" class="thcell">type</td>
			</tr>
			<xsl:for-each select="request/view/entry">
				<tr>
					...
					<td>
						<a title="{name}">
							<xsl:attribute name="href">Provider?type=get_xml&amp;xmlname=<xsl:value-of select="name" />&amp;typerule=<xsl:value-of
