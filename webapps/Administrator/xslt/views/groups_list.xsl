...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="type">get_groups_list</xsl:variable>
	<xsl:variable name="doctype">groups</xsl:variable>
	...
	<xsl:template match="/request">
		<html>
			<head>
				<title>Administrator - Groups list</title>
									...
									<tr onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
										...
										<td style="border: 1px solid #ccc; padding-left:5px">
											<a class="doclink" title="{@id}">
												<xsl:attribute name="href">Provider?type=get_group&amp;docid=<xsl:value-of select="@docid"/>&amp;dbid=<xsl:value-of select="@dbid"/></xsl:attribute>
