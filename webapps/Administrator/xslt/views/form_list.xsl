...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<xsl:call-template name="layout">
			<xsl:with-param name="w_title" select="'Administrator - Forms list'" />
		</xsl:call-template>
	</xsl:template>
				...
				<tr onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
					...
					<td style="border:1px solid #ccc; padding-left:5px;">
						<a class="doclink" title="{filename}">
							<xsl:attribute name="href">Provider?type=get_query&amp;id=<xsl:value-of select="@docid" />&amp;app=<xsl:value-of
