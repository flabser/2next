...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<xsl:call-template name="layout">
			<xsl:with-param name="w_title" select="concat('Administrator ',@id)" />
		</xsl:call-template>
	</xsl:template>
				...
				<tr>
					...
					<td style="border:1px solid #ccc; padding-left:5px; word-wrap:break-word">
						<a class="doclink" title="{filename}">
							<xsl:attribute name="href">Provider?type=edit&amp;element=schedule&amp;id=<xsl:value-of
					...
					<td class="title" style="border:1px solid #ccc; text-align:center">
						<a style="margin-left:10" title="run handler unscheduled...">
							<xsl:attribute name="href">Provider?type=service&amp;operation=do_scheduled_handler&amp;id=<xsl:value-of
								select="id" /></xsl:attribute>
