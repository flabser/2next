...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="doctype">maindoc</xsl:variable>
	<xsl:variable name="viewtype">Документы</xsl:variable>
	...
	<xsl:template match="/request">
		<html>
			<head>
				<title>Administrator - Maindocs list</title>
									...
									<tr onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
										...
										<td style="border: 1px solid #ccc; padding-left:5px">
											<a class="doclink" title="{@docid}">
												<xsl:attribute name="href">Provider?type=edit&amp;element=document&amp;id=<xsl:value-of select="@docid"/>&amp;dbid=<xsl:value-of select="@dbid"/></xsl:attribute>
