...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:variable name="doctype" select="'execution'" />
	...
	<xsl:template match="/request">
		<xsl:call-template name="layout">
			<xsl:with-param name="w_title" select="'Administrator - Executions list'" />
		</xsl:call-template>
	</xsl:template>

	<xsl:template name="_content">
		<h4 class="view-header-title">
			<xsl:value-of select="concat(query/@dbid,' Executions')" />
		</h4>
		<div class="actionbar">
			<span class="action">
				...
				<tr onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
					...
					<td style="border:1px solid #ccc; text-align:center">
						<input type="checkbox" name="chbox">
							...
							<xsl:attribute name="class" select="@doctype" />
						</input>
					</td>
					<td style="border: 1px solid #ccc; padding-left:5px">
						<a class="doclink" title="{@docid}">
							<xsl:attribute name="href">Provider?type=get_execution&amp;docid=<xsl:value-of select="@docid" />&amp;dbid=<xsl:value-of
