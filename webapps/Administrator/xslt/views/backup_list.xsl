...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="/request">
		<xsl:call-template name="layout">
			<xsl:with-param name="w_title" select="'Administrator - Backup'" />
		</xsl:call-template>
	</xsl:template>
			...
			<div class="view-content">
				<table class="viewtable">
					<tr class="th" style="height:32px">
						...
						<td class="thcell">Database</td>
						<td class="thcell">Last modified</td>
						...
						<tr onmouseover="javascript:elemBackground(this,'EEEEEE')" onmouseout="elemBackground(this,'FFFFFF')">
							...
							<td style="border:1px solid #ccc; padding-left:5px">
								<a class="doclink" title="{name}">
									<xsl:attribute name="href">Provider?type=edit&amp;element=backup&amp;id=<xsl:value-of
										select="name" />&amp;app=<xsl:value-of select="app" /></xsl:attribute>
