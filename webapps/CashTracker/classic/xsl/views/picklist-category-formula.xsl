...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="entry" mode="label">
		<label class="dialog-list-item" ondblclick="nb.dialog.execute(this)">
			...
			<input data-type="select" type="radio" name="{$select-name}" value="{@docid}" data-id="{@id}" data-text="{viewcontent/viewtext1}" />
	...
	<xsl:template match="entry" mode="label-response">
		<label class="dialog-list-item" ondblclick="nb.dialog.execute(this)">
			...
			<input data-type="select" type="radio" name="{$select-name}" value="{@docid}" data-id="{@id}" data-text="{viewtext}"
				class="js-response" />
			<xsl:if test="hasresponse='true'">
				<xsl:call-template name="glossResponse" />
			</xsl:if>
